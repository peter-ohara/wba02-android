...
var counter=0;function AMparseMath(str){str=str.replace(/(&nbsp;|\u00a0|&#160;)/g,"");str=str.replace(/&gt;/g,">");str=str.replace(/&lt;/g,"<");str=str.replace(/×/g,"\\times");str=str.replace(/−/g,"-");str=str.replace(/&Omega/g,"\\Omega");str=str.replace(/\/\//g,"text(/)");str=str.replace(/’/g,"text(')");str=str.replace(/”/g,'text(")');str=str.replace(/·/g,"\\cdot");if(str.match(/\S/)==null){return document.createTextNode(" ");}
...
texstring=texstring.replace('$','\\$');texstring=texstring.replace(/%/g,'\\%');texstring=texstring.replace(/∠/g,'\\angle');texstring=texstring.replace(/Φ/g,"\\Phi");texstring=texstring.replace(/θ/g,"\\theta");texstring=texstring.replace(/Δ/g,"\\Delta");texstring=texstring.replace(/α/g,"\\alpha");texstring=texstring.replace(/β/g,"\\beta");texstring=texstring.replace(/ω/g,"\\omega");texstring=texstring.replace(/μ/g,"\\mu");texstring=texstring.replace(/Ω/g,"\\Omega");texstring=texstring.replace(/δ/g,"\\delta");texstring=texstring.replace(/γ/g,"\\gamma");texstring=texstring.replace(/σ/g,"\\sigma");texstring=texstring.replace(/π/g,"\\pi");texstring=texstring.replace(/≥/g,"\\ge");texstring=texstring.replace(/≤/g,"\\le");texstring=texstring.replace(/≠/g,"\\ne");texstring=texstring.replace(/≡/g,"\\equiv");texstring=texstring.replace(/≈/g,"\\approx");texstring=texstring.replace(/→/g,"\\rightarrow");texstring=texstring.replace(/÷/g,"\\div");texstring=texstring.replace(/±/g,"\\pm");texstring=texstring.replace(/°/g,"^{\\circ}");texstring=texstring.replace(/∞/g,"\\infty");var node=document.createElement("span");node.id="mathId"+counter;node.innerHTML=texstring;try{if(str.indexOf('\\begin{cases}')>-1){katex.render(str,node);}else{katex.render(texstring,node);}}catch(err){node.className="katexErr";if(typeof MathJax!=='undefined'){node.innerHTML="`"+str+"`";MathJax.Hub.Queue(["Typeset",MathJax.Hub,"mathId"+counter]);}else{node.innerHTML=err;}}
counter++;return node;}
function AMTparseMath(str){return AMparseMath(str);}
function AMstrarr2docFrag(arr,linebreaks){var newFrag=document.createDocumentFragment();var expr=false;for(var i=0;i<arr.length;i++){if(expr)newFrag.appendChild(AMparseMath(arr[i]));else{var arri=(linebreaks?arr[i].split("\n\n"):[arr[i]]);newFrag.appendChild(document.createElement("span").appendChild(document.createTextNode(arri[0])));for(var j=1;j<arri.length;j++){newFrag.appendChild(document.createElement("p"));newFrag.appendChild(document.createElement("span").appendChild(document.createTextNode(arri[j])));}}
expr=!expr;}
return newFrag;}
function AMprocessNodeR(n,linebreaks){var mtch,str,arr,frg,i;if(n.childNodes.length==0){if((n.nodeType!=8||linebreaks)&&n.parentNode.nodeName!="form"&&n.parentNode.nodeName!="FORM"&&n.parentNode.nodeName!="textarea"&&n.parentNode.nodeName!="TEXTAREA"&&n.parentNode.nodeName!="pre"&&n.parentNode.nodeName!="PRE"&&n.parentNode.nodeName!="code"&&n.parentNode.nodeName!="CODE"){str=n.nodeValue;if(!(str==null)){str=str.replace(/\r\n\r\n/g,"\n\n");if(doubleblankmathdelimiter){str=str.replace(/\x20\x20\./g," "+AMdelimiter1+".");str=str.replace(/\x20\x20,/g," "+AMdelimiter1+",");str=str.replace(/\x20\x20/g," "+AMdelimiter1+" ");}
...
str=str.replace(new RegExp(AMescape1,"g"),function(st){mtch=true;return"AMescape1"});if(AMusedelimiter2)str=str.replace(new RegExp(AMdelimiter2regexp,"g"),AMdelimiter1);arr=str.split(AMdelimiter1);for(i=0;i<arr.length;i++)
if(AMusedelimiter2){arr[i]=arr[i].replace(/AMescape2/g,AMdelimiter2).replace(/AMescape1/g,AMdelimiter1);}else{arr[i]=arr[i].replace(/AMescape1/g,AMdelimiter1);}
if(arr.length>1||mtch){frg=AMstrarr2docFrag(arr,n.nodeType==8);var len=frg.childNodes.length;n.parentNode.replaceChild(frg,n);return len-1;}}}else return 0;}else if(n.nodeName!="math"){for(i=0;i<n.childNodes.length;i++)
...
function translate(spanclassAM){if(!AMtranslated){AMtranslated=true;var body=document.getElementsByTagName("body")[0];var processN=document.getElementById(AMdocumentId);AMprocessNode((processN!=null?processN:body),false,spanclassAM);}}
var AMbody;var AMtranslated=false;var AMnoMathML=true;AMinitSymbols();window.translate=translate;window.AMprocessNode=AMprocessNode;window.AMparseMath=AMparseMath;window.AMTparseMath=AMparseMath;window.AMTparseAMtoTeX=AMTparseAMtoTeX;if(typeof(doAMprocessor)!="undefined"){translate();};})();